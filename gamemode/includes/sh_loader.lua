...
/*------------------------------------
	_L
------------------------------------*/
local function _L( func, path, files, ignore, callback, islua )

	// load all files by default
	files = files || "*";

	// gather files
	local filelist = func( ("%s%s"):format( path, files ), islua and "LUA" or "GAME" );
...
function LL( path, files, ignore, callback )
	...
	_L( file.Find, path, files, ignore, callback, true );
