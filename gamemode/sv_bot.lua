...
function GM:BotFindPad(ball, goal, ignore, ignoreLOS)
	...
	for _, pad in pairs(pads) do
		...
		if (dist1 + dist2) < distanceToBeat and dist3 > dist1 then
			if ignoreLOS or (not ignoreLOS and self:BotCheckLOS(ballPos, pos, {ball, pad})) then
...
function GM:BotCalculateStroke(ball)
	...
	if rules.Call("CanBallSink", ball) then
		...
		if self:BotCheckLOS(ballPos, cupPos, {ball, cup}) then
			...
			if IsValid(pad) then
				...
				if self:BotCheckLOS(ballPos, padPos, {ball, pad}) then
					...
					runnerUp[#runnerUp + 1] = pad
				end
			else
				runnerUp[#runnerUp + 1] = cup
			end
		end
	else
		...
		for _, ring in pairs(rings) do
			...
			if not ring:IsTeamDone(ball:Team()) then
	...
	if not IsValid(closestGoal) then
...
function GM:BotCheckLOS(from, to, filter)
	...
	if tr.Fraction == 1 then
		...
		for i = 1, 16 do
			...
			if tr2.HitSky or tr.Fraction < tr2.Fraction then
				return false
			end
		end
...
function GM:BotHit(bot)
	...
	if not bot:CanHit() then return end
